...
- **Обеспечить связь** между данными о продажах и соответствующими акцизным маркам в системе 1С.
...
- **Задача на доработку: Автоматический поиск связанных чеков по разным точкам продаж.** При выполнении загрузки или сверки по выбранным точкам продаж система должна автоматически находить все чеки, связанные с ними по `checkid`, даже если они относятся к другим точкам продаж, не указанным в первоначальном фильтре. Это обеспечит полную загрузку и корректную сверку объединенных чеков.
...
## Корректное распределение скидки по позициям чека

### Контекст
В текущей реализации SQL-запроса (`Script-2.sql`) для получения данных о продажах из Oracle, скидка на чек распределяется пропорционально на все позиции в чеке. Однако, бизнес-логика предполагает, что скидка может применяться только к позициям, предшествующим строке самой скидки в чеке. Это приводит к некорректному расчету стоимости отдельных позиций.

### Задача
Адаптировать SQL-запрос `Script-2.sql` для корректного распределения суммы скидки только на те строки, которые предшествуют строке скидки в чеке.

### Требования к реализации
1.  **Изменить логику расчета `СуммаСкидкиСтроки`**:
    -   Скидка (`-agg.misum`) должна распределяться только на те товарные позиции (`cd`), у которых порядковый номер `cd.detailindex` меньше, чем порядковый номер строки скидки `agg.detailindex`.
    -   Для позиций, которые находятся на уровне или после строки скидки (`cd.detailindex >= agg.detailindex`), сумма скидки на строку должна быть равна 0.
2.  **Пропорциональное распределение**:
    -   Сумма скидки должна распределяться пропорционально стоимости каждой подходящей товарной позиции.
    -   Для этого необходимо сначала рассчитать общую сумму всех позиций, на которые распространяется скидка.
3.  **Использование оконных функций**:
    -   Для реализации расчета рекомендуется использовать оконные функции SQL (например, `SUM() OVER (...)`), чтобы вычислить сумму для распределения скидки без усложнения общей структуры запроса.
