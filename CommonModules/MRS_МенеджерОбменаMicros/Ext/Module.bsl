...
Процедура ФоновоеЗаданиеЗагрузкаЧеков(ФронтСистема, Пользователь) Экспорт
	...
	Если ТипЗнч(ДатаЗапрета) = Тип("Структура") Тогда 
		...
		ВыполнитьЗагрузкуДанныхMicros(ФронтСистема, ДатаЗапрета.НачалоПериода, ДатаЗапрета.КонецПериода,,0);		
...
Процедура ВыполнитьЗагрузкуДанныхMicros(ФронтСистема, НачалоПериода, КонецПериода, ПараметрыКасс = Неопределено, НомерЧека) Экспорт
