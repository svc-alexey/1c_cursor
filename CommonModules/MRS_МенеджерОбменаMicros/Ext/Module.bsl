...
Функция ПолучениеДанныхORACLE(ПараметрыВыборки)
	...
	CommandText = "-- CTE TenderData: Получение информации о методах оплаты для каждого чека.
				|-- Выбирается основной (максимальный) номер метода оплаты для каждого чека.
				|WITH TenderData AS (
				|    SELECT 
				|        c.checkid,
				...
				|    GROUP BY c.checkid
				|),
				|-- CTE DiscountData: Расчет общей суммы скидки и получение наименования скидки для каждого чека.
				|DiscountData AS (
				|    SELECT 
				|        c.checkid,
				...
				|    GROUP BY c.checkid
				|),
				|-- CTE AggregatedDiscountData: Объединение данных об оплатах и скидках по чекам.
				...
				|),
				|-- CTE PreChecksDetail: Основной подзапрос для выборки детализированных данных по товарным строкам чеков.
				|-- Соединяет множество таблиц для получения полной информации о каждой позиции.
				|PreChecksDetail AS (
				|    SELECT
				|        d.checkid,
				|		 d.detailindex,
				...
				|),
				|-- CTE ChecksDetail: Обработка данных из PreChecksDetail.
				|-- Рассчитывает общую сумму продаж по чеку с использованием оконной функции.
				|ChecksDetail AS (
				|    SELECT
				|        checkid,
				|        stringobjectnum,
				...
				|    WHERE quantity != 0
				|)
				|-- Финальный SELECT: Сборка итогового результата.
				|-- Объединяет данные по товарным строкам с данными об оплатах и скидках.
				|-- Рассчитывает сумму скидки, приходящуюся на каждую товарную строку.
				|SELECT
