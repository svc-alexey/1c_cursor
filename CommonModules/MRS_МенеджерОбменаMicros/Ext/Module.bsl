...
Процедура ДобавитьЧекиКСверке(ТаблицаЗначенийORACLE, ПараметрыВыборки) 
	...
	Запрос.Текст = "ВЫБРАТЬ
	               ...
	               |	ТаблицаORACLE.ИДЧекаКАК ИДЧекаORACLE,
	               ...
	               |			И ТаблицаORACLE.НомерПозицииВЧеке = питДанныеПродажФронтаТовары.MRS_ПозицияВЧеке
					|		ЛЕВОЕ СОЕДИНЕНИЕ ОбъединенныеЧекиИД КАК Объединенные // MRS-XXX
					|			ПО питДанныеПродажФронтаТовары.Ссылка.MRS_IDOracle = Объединенные.MRS_IDOracle
					|			И НАЧАЛОПЕРИОДА(питДанныеПродажФронтаТовары.Ссылка.Дата, ДЕНЬ) = Объединенные.ДатаЧека
	               |";
