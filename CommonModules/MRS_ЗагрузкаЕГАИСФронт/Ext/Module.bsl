...
Функция ПолучитьНастройкиФискальнойБазы(ИмяФискальнойБазы, Пользователь) Экспорт
	...
	Если РезультатЗапроса.Количество() = 0 Тогда
		...
		ТекстСообщения = СтрШаблон("Не удалось получить настройку подключения, проверьте РегистрСведений.MRS_НастройкиПодключенияFiscalDBFront. Фискальная база: %1",ИмяФискальнойБазы);
		ЗаписатьВЖурналРегистрации(ТекстСообщения,,Истина);	
	...
	Пока РезультатЗапроса.Следующий() Цикл
		...
		ДатаЗапретаЗагрузкиФискальня = РезультатЗапроса.ДатаЗапретаЗагрузки;
    ...
    Иначе
	    СтруктураНастроек.Вставить("ДатаЗапретаЗагрузки", ДатаЗапретаЗагрузкиФискальня);
	КонецЕсли;
...
Функция ПолучитьДатыЗапрета(Пользователь)
	...
	Текст = "ВЫБРАТЬ
	        |	НАЧАЛОПЕРИОДА(МИНИМУМ(Даты.ДатаЗапрета), ДЕНЬ) КАК ДатаЗапрета
	        |ИЗ
	        |	(ВЫБРАТЬ
...
Процедура ПолучитьРезультатыЗапросаКФискальнойБазе(ДанныеФискальнойБазы, НастройкиФискальнойБазы) Экспорт
	
	ВнешнийИсточникФискальнаяБаза = Неопределено;
	
	Если НастройкиФискальнойБазы.ИмяВнешнейБД = "MRS_FISCAL_BASE_MRIYA" Тогда
		
		ВнешнийИсточникФискальнаяБаза = ВнешниеИсточникиДанных.MRS_FISCAL_BASE_MRIYA;
		
	ИначеЕсли НастройкиФискальнойБазы.ИмяВнешнейБД = "MRS_FISCAL_BASE_WINEPARK" Тогда
		
		ВнешнийИсточникФискальнаяБаза = ВнешниеИсточникиДанных.MRS_FISCAL_BASE_WINEPARK;
		
	ИначеЕсли НастройкиФискальнойБазы.ИмяВнешнейБД = "MRS_FISCAL_BASE_AIPETRI" Тогда
		
		ВнешнийИсточникФискальнаяБаза = ВнешниеИсточникиДанных.MRS_FISCAL_BASE_AIPETRI;
		
	ИначеЕсли НастройкиФискальнойБазы.ИмяВнешнейБД = "MRS_FISCAL_BASE_K32MOSCOW" Тогда
		
		ВнешнийИсточникФискальнаяБаза = ВнешниеИсточникиДанных.MRS_FISCAL_BASE_K32MOSCOW;
		
	ИначеЕсли НастройкиФискальнойБазы.ИмяВнешнейБД = "MRS_FISCAL_BASE_CHAIKAMOSCOW" Тогда
		
		ВнешнийИсточникФискальнаяБаза = ВнешниеИсточникиДанных.MRS_FISCAL_BASE_CHAIKAMOSCOW;
		
	ИначеЕсли НастройкиФискальнойБазы.ИмяВнешнейБД = "MRS_FISCAL_BASE_KHERSONES" Тогда
		
		ВнешнийИсточникФискальнаяБаза = ВнешниеИсточникиДанных.MRS_FISCAL_BASE_KHERSONES;
		
	ИначеЕсли НастройкиФискальнойБазы.ИмяВнешнейБД = "MRS_FISCAL_BASE_AZUR_2" Тогда
		
		ВнешнийИсточникФискальнаяБаза = ВнешниеИсточникиДанных.MRS_FISCAL_BASE_AZUR_2;
		
	ИначеЕсли НастройкиФискальнойБазы.ИмяВнешнейБД = "MRS_FISCAL_BASE_ALUPKA" Тогда
		
		ВнешнийИсточникФискальнаяБаза = ВнешниеИсточникиДанных.MRS_FISCAL_BASE_ALUPKA;	
	...
	Попытка
		...
		ПараметрыСоединения.СтрокаСоединения = "DRIVER=SQLite3 ODBC Driver;Database=\\" + НастройкиФискальнойБазы.Сервер + "\fiscaldb\1C_DB\FiscalDB.db3"; 
		...
		Для Каждого Запись из Набор Цикл
			...
			Запись.ДатаЗапретаЗагрузки = ТекущаяДата();
	...
	Исключение
		...
		ЗаписатьВЖурналРегистрации(ОписаниеОшибки, , Истина);
...
//вызов функции из общего модуля питТиповыеДокументы
...
Процедура ОбработатьПолученныеДанные(ДанныеФискальнойБазы)
	...
	Для Каждого СтрокаБД Из ДанныеФискальнойБазы Цикл
	    СтрокаБД.code = ОчиститьНедопустимыеСимволыXML(СтрокаБД.code);
	КонецЦикла;
	...
	|			ТОГДА ТТНВходящаяЕГАИСТовары.Серия
	|		КОГДА ТТНВходящаяЕГАИСТовары.Ссылка.ДокументОснование ССЫЛКА Документ.ПриобретениеТоваровУслуг
	|			ТОГДА ПриобретениеТоваровУслугТовары.Серия
	|		КОГДА ТТНВходящаяЕГАИСТовары.Ссылка.ДокументОснование ССЫЛКА Документ.ПеремещениеТоваров
	...
	|				ПО ТТНВходящаяЕГАИСТовары.Ссылка.ДокументОснование = ПеремещениеТоваровТовары.Ссылка
	|					И ТТНВходящаяЕГАИСТовары.Номенклатура = ПеремещениеТоваровТовары.Номенклатура
...
// Функция для очистки недопустимых символов
Функция ОчиститьНедопустимыеСимволыXML(Знач ВходнаяСтрока)
    Результат = "";
    Для НомерСимвола = 1 По СтрДлина(ВходнаяСтрока) Цикл
        Символ = Сред(ВходнаяСтрока, НомерСимвола, 1);
        КодСимвола = КодСимвола(Символ);
        // Оставляем только допустимые символы (коды >= 32, плюс \t, \n, \r)
        Если КодСимвола >= 32 Или КодСимвола = 9 Или КодСимвола = 10 Или КодСимвола = 13 Тогда
