...
3. **Для каждой найденной позиции будет создано**:
   - Серийная номенклатура с суффиксом `[mrs]` и указанием объема в литрах
   - Упаковка в штуках с коэффициентом пересчета (объем × 10)
   - Настройки единицы для отчетов и коэффициента
   - Запись в регистре `ОписаниеНоменклатурыИС` для интеграции с информационными системами
4. **Специальная обработка** для номенклатуры без остатков:
   - Изменение единицы хранения остатков на литры
   - Создание упаковки и настроек ИС
5. **Автоматическая коррекция наименований**: добавление объема в правильном формате (например, "0,5л", "1,5л")
6. Дождитесь завершения процесса (отображается прогресс по пакетам)
