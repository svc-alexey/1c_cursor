...
Процедура ВыполнитьНаСервере()
	...
	|				И РазделУчета = ЗНАЧЕНИЕ(Перечисление.РазделыУчетаСебестоимостиТоваров.ТоварыНаСкладах)) КАК СебестоимостьТоваровОстатки
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
...
Процедура ПроверитьОтрицательныеОстаткиНаСервере(ДатаПроверки)
    ...
    |ИЗ
    |	РегистрНакопления.СебестоимостьТоваров.Остатки(&Дата, РазделУчета = ЗНАЧЕНИЕ(Перечисление.РазделыУчетаСебестоимостиТоваров.ТоварыНаСкладах)) КАК Остатки
    |ГДЕ
    ...
    |ИЗ
    |	РегистрНакопления.ТоварыОрганизаций.Остатки(&Дата, ) КАК Остатки
    |ГДЕ
    |	Остатки.КоличествоОстаток < 0
    |
    |ОБЪЕДИНИТЬ ВСЕ
