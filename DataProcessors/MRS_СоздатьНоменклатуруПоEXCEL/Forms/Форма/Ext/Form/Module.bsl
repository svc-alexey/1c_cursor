...
Процедура ПредзаполнитьСвязиРецептурНаСервере()
	...
	Запрос.Текст = 
		"ВЫБРАТЬ
		|	СтарыеРецептуры.Объект КАК СтараяРецептура,
		|	СтарыеРецептуры.Значение КАК НомерНовойРецептуры,
		|	СтарыеРецептуры.Объект.Наименование КАК НаименованиеСтарой,
		|	СтарыеРецептуры.Объект.Номер КАК НомерСтарой,
		|	СтарыеРецептуры.Объект.Подразделение КАК Подразделение
		|ИЗ
		|	Справочник.питРецептура.ДополнительныеРеквизиты КАК СтарыеРецептуры
		|ГДЕ
		|	СтарыеРецептуры.Свойство.Наименование = 'MRS_НоваяРецептура'
		|	И НЕ СтарыеРецептуры.Объект.ПометкаУдаления
		|	И СтарыеРецептуры.Объект.ДатаКонца = ДАТАВРЕМЯ(1, 1, 1, 0, 0, 0)
		|
		|УПОРЯДОЧИТЬ ПО
		|	НомерСтарой";
...
Процедура ОбновитьДеревоРецептурНаСервере()
	...
	// Сначала получим пары рецептур из таблицы Рецептуры (старая-новая)
	ПарыРецептур = Новый Соответствие;
	Для Каждого СтрокаРецептуры Из Рецептуры Цикл
		Если ЗначениеЗаполнено(СтрокаРецептуры.РецептураСтарая) И ЗначениеЗаполнено(СтрокаРецептуры.РецептураНовая) Тогда
			ПарыРецептур.Вставить(СтрокаРецептуры.РецептураСтарая, СтрокаРецептуры.РецептураНовая);
		КонецЕсли;
	КонецЦикла;
	
	// Основной запрос для получения рецептур с вскрытой номенклатурой
	Запрос = Новый Запрос;
	Запрос.Текст = 
		"ВЫБРАТЬ
		|	питРецептура.Ссылка КАК РецептураСсылка,
		|	питРецептура.Наименование КАК РецептураНаименование,
		|	питРецептура.Номер КАК РецептураНомер,
		|	питРецептураИнгредиенты.Номенклатура КАК ВскрытаяНоменклатура,
		|	питРецептураИнгредиенты.Номенклатура.Наименование КАК ИнгредиентНаименование,
		|	питРецептураИнгредиенты.Количество КАК Количество,
		|	MRS_СоответствиеСтаройНовойНоменклатуры.НоваяНоменклатура КАК НоваяНоменклатура
		|ИЗ
		|	Документ.питРецептура КАК питРецептура
		|		ЛЕВОЕ СОЕДИНЕНИЕ Документ.питРецептура.Ингредиенты КАК питРецептураИнгредиенты
		|		ПО питРецептура.Ссылка = питРецептураИнгредиенты.Ссылка
		|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.MRS_СоответствиеСтаройНовойНоменклатуры КАК MRS_СоответствиеСтаройНовойНоменклатуры
		|		ПО питРецептураИнгредиенты.Номенклатура = MRS_СоответствиеСтаройНовойНоменклатуры.ВскрытаяНоменклатура
		|ГДЕ
		|	питРецептура.ДатаКонца = ДАТАВРЕМЯ(1, 1, 1, 0, 0, 0)
		|	И НЕ питРецептура.ПометкаУдаления
		|	И питРецептураИнгредиенты.Номенклатура.АлкогольнаяПродукцияВоВскрытойТаре
		|	И питРецептураИнгредиенты.Номенклатура.АлкогольнаяПродукция";
	
	// Добавим фильтр, если установлен флажок "Только без соответствий"
	Если ТолькоБезСоответствий Тогда
		Запрос.Текст = Запрос.Текст + "
		|	И MRS_СоответствиеСтаройНовойНоменклатуры.НоваяНоменклатура ЕСТЬ NULL";
	КонецЕсли;
	
	Запрос.Текст = Запрос.Текст + "
	|
	|УПОРЯДОЧИТЬ ПО
	|	РецептураНаименование,
	|	ИнгредиентНаименование";
	
	РезультатЗапроса = Запрос.Выполнить();
	Выборка = РезультатЗапроса.Выбрать();
	
	// Группируем по старым рецептурам
	ОбработанныеПары = Новый Соответствие;
	
	Пока Выборка.Следующий() Цикл
		
		СтараяРецептура = Выборка.РецептураСсылка;
		НоваяРецептура = ПарыРецептур[СтараяРецептура];
		
		// Ключ для группировки пар
		КлючПары = Строка(СтараяРецептура);
		
		Если ОбработанныеПары[КлючПары] = Неопределено Тогда
			
			// Создаем корневой узел для пары рецептур
			УзелПары = ДеревоРецептур.ПолучитьЭлементы().Добавить();
			УзелПары.Наименование = "Рецептура №" + Выборка.РецептураНомер + " (" + СокрЛП(Выборка.РецептураНаименование) + ")";
			УзелПары.ТипУзла = "ПараРецептур";
			УзелПары.РецептураСсылка = СтараяРецептура;
			
			// Добавляем старую рецептуру
			УзелСтарой = УзелПары.ПолучитьЭлементы().Добавить();
			УзелСтарой.Наименование = "СТАРАЯ: " + Выборка.РецептураНаименование;
			УзелСтарой.РецептураСсылка = СтараяРецептура;
			УзелСтарой.ТипУзла = "СтараяРецептура";
			УзелСтарой.СтатусСоответствия = "Исходная";
			
			// Добавляем новую рецептуру (если есть)
			Если ЗначениеЗаполнено(НоваяРецептура) Тогда
				УзелНовой = УзелПары.ПолучитьЭлементы().Добавить();
				УзелНовой.Наименование = "НОВАЯ: " + НоваяРецептура.Наименование;
				УзелНовой.РецептураСсылка = НоваяРецептура;
				УзелНовой.ТипУзла = "НоваяРецептура";
				УзелНовой.СтатусСоответствия = "Создана";
			Иначе
				УзелНовой = УзелПары.ПолучитьЭлементы().Добавить();
			...
			ОбработанныеПары.Вставить(КлючПары, Новый Структура("УзелПары, УзелСтарой, УзелНовой", УзелПары, УзелСтарой, УзелНовой));
			
		КонецЕсли;
		
		УзлыПары = ОбработанныеПары[КлючПары];
		
		// Добавляем ингредиент к старой рецептуре
		УзелИнгредиента = УзлыПары.УзелСтарой.ПолучитьЭлементы().Добавить();
		УзелИнгредиента.Наименование = Выборка.ИнгредиентНаименование;
		УзелИнгредиента.СтараяНоменклатура = Выборка.ВскрытаяНоменклатура;
		УзелИнгредиента.НоваяНоменклатура = Выборка.НоваяНоменклатура;
		УзелИнгредиента.Количество = Выборка.Количество;
		УзелИнгредиента.РецептураСсылка = Выборка.РецептураСсылка;
	...
	// Теперь добавляем полный состав новых рецептур
	Для Каждого ЭлементПары Из ОбработанныеПары Цикл
		
		УзлыПары = ЭлементПары.Значение;
		НоваяРецептура = ПарыРецептур[УзлыПары.УзелПары.РецептураСсылка];
		
		Если ЗначениеЗаполнено(НоваяРецептура) Тогда
			
			НоваяРецептураОбъект = НоваяРецептура.ПолучитьОбъект();
			Для Каждого СтрокаИнгредиентаНовой Из НоваяРецептураОбъект.Ингредиенты Цикл
				
				УзелИнгредиентаНовой = УзлыПары.УзелНовой.ПолучитьЭлементы().Добавить();
				УзелИнгредиентаНовой.Наименование = СтрокаИнгредиентаНовой.Номенклатура.Наименование;
				
				// Попробуем найти соответствие в старой рецептуре по вскрытой номенклатуре
				СтараяВскрытая = Неопределено;
				...
				Запрос2.Текст = 
					...
					|	РегистрСведений.MRS_СоответствиеСтаройНовойНоменклатуры КАК MRS_СоответствиеСтаройНовойНоменклатуры
					|ГДЕ
					|	MRS_СоответствиеСтаройНовойНоменклатуры.НоваяНоменклатура = &НоваяНоменклатура";
				Запрос2.УстановитьПараметр("НоваяНоменклатура", СтрокаИнгредиентаНовой.Номенклатура);
				РезультатЗапроса2 = Запрос2.Выполнить();
				Выборка2 = РезультатЗапроса2.Выбрать();
				Если Выборка2.Следующий() Тогда
					СтараяВскрытая = Выборка2.ВскрытаяНоменклатура;
				КонецЕсли;
				
				УзелИнгредиентаНовой.СтараяНоменклатура = СтараяВскрытая;
				УзелИнгредиентаНовой.НоваяНоменклатура = СтрокаИнгредиентаНовой.Номенклатура;
				УзелИнгредиентаНовой.Количество = СтрокаИнгредиентаНовой.Количество;
				УзелИнгредиентаНовой.РецептураСсылка = НоваяРецептура;
				УзелИнгредиентаНовой.ТипУзла = "ИнгредиентНовый";
				
				Если ЗначениеЗаполнено(СтараяВскрытая) Тогда
					УзелИнгредиентаНовой.СтатусСоответствия = "Заменён";
				Иначе
					УзелИнгредиентаНовой.СтатусСоответствия = "Новый";
				КонецЕсли;
