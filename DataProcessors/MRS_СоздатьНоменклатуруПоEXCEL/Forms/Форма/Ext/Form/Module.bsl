...
Процедура ОбновитьДеревоРецептурНаСервере()
	...
	ДеревоРецептур.Строки.Очистить();
	...
	Для Каждого СтрокаПары Из Рецептуры Цикл
		...
		УзелПары = ДеревоРецептур.Строки.Добавить();
		УзелПары.ТипУзла = "ПараРецептур";
		УзелПары.РецептураСсылка = СтрокаПары.РецептураСтарая; // Используем существующую колонку "РецептураСсылка"
...
Процедура ЗаполнитьУзелИнгредиентами(УзелПары, СсылкаНаСтарую, СсылкаНаНовую, ТЗИнгредиентов)
	...
	Для Каждого СтрокаСтарогоИнгредиента Из НайденныеИнгредиентыСтарые Цикл
		УзелИнгредиента = УзелПары.Строки.Добавить();
		УзелИнгредиента.ТипУзла = "Ингредиент";
		
		// Заполняем данные по старому ингредиенту, используя ПРАВИЛЬНЫЕ имена колонок из XML
		УзелИнгредиента.СтараяНоменклатура = СтрокаСтарогоИнгредиента.Номенклатура;
		УзелИнгредиента.Количество = СтрокаСтарогоИнгредиента.Количество;
		...
		Если СоответствующийНовый <> Неопределено Тогда
			...
			УзелИнгредиента.НоваяНоменклатура = СоответствующийНовый.Номенклатура;
			// У нас нет отдельной колонки для нового количества, предположим, оно то же
		...
		Если СтрокаСтарогоИнгредиента.ЭтоВскрытаяТара Тогда
			Если ЗначениеЗаполнено(УзелИнгредиента.НоваяНоменклатура) ИЛИ ЗначениеЗаполнено(СтрокаСтарогоИнгредиента.НоваяНоменклатура) Тогда
				УзелИнгредиента.СтатусСоответствия = "Есть соответствие";
				Если НЕ ЗначениеЗаполнено(УзелИнгредиента.НоваяНоменклатура) Тогда
					УзелИнгредиента.НоваяНоменклатура = СтрокаСтарогоИнгредиента.НоваяНоменклатура;
				КонецЕсли;
			Иначе
	...
	Для Каждого ЭлементКарты Из КартаНовыхИнгредиентов Цикл
		...
		УзелИнгредиента.НоваяНоменклатура = СтрокаНовогоИнгредиента.Номенклатура;
		УзелИнгредиента.Количество = СтрокаНовогоИнгредиента.Количество;
...
Процедура СохранитьИзмененияВРецептурахНаСервере()
	...
	Для Каждого УзелПары Из ДеревоРецептур.Строки Цикл
		
		// Новую рецептуру теперь нужно получать из старой, так как в дереве нет прямого поля
		СсылкаНаСтаруюРецептуру = УзелПары.РецептураСсылка;
		// ... здесь нужна логика получения новой рецептуры по старой ...
		// Предположим пока, что мы ее как-то получили
		СсылкаНаНовуюРецептуру = Неопределено; // ЗАГЛУШКА
		
		Для Каждого УзелИнгредиента Из УзелПары.Строки Цикл
			
			// Собираем изменения только если есть новая номенклатура И есть новая рецептура для изменения
			ЭтоСтрокаИнгредиента = УзелИнгредиента.ТипУзла = "Ингредиент";
			ЕстьНоваяНоменклатура = ЗначениеЗаполнено(УзелИнгредиента.НоваяНоменклатура);
			ЕстьСтараяНоменклатура = ЗначениеЗаполнено(УзелИнгредиента.СтараяНоменклатура);
			ЕстьНоваяРецептура = ЗначениеЗаполнено(СсылкаНаНовуюРецептуру);
			
			Если ЕстьНоваяНоменклатура И ЭтоСтрокаИнгредиента И ЕстьСтараяНоменклатура И ЕстьНоваяРецептура Тогда // Используем полученную ссылку
				...
				СтрокаИзменения.РецептураСсылка = СсылкаНаНовуюРецептуру; 
	...
	Попытка
		...
		// 1. Сначала сохраняем все соответствия в регистр сведений
		Для Каждого СтрокаИзменения Из ТаблицаИзменений Цикл
			...
			// Если запись уже есть, обновляем, иначе создаем новую
		...
		// 2. Группируем изменения по рецептурам, чтобы избежать многократного получения и записи одного и того же объекта
		СгруппированныеИзменения = ТаблицаИзменений.Скопировать();
		СгруппированныеИзменения.Свернуть("РецептураСсылка");
		
		// 3. Теперь обновляем сами рецептуры
		Для Каждого СтрокаГруппы Из СгруппированныеИзменения Цикл
			
			РецептураОбъект = СтрокаГруппы.РецептураСсылка.ПолучитьОбъект();
			БылиИзмененияВРецептуре = Ложь;
			
			// Отбираем все изменения для текущей рецептуры
			Отбор = Новый Структура("РецептураСсылка", СтрокаГруппы.РецептураСсылка);
			СтрокиДляЭтойРецептуры = ТаблицаИзменений.НайтиСтроки(Отбор);
			
			// Создаем карту замен для быстрого поиска
			КартаЗамен = Новый Соответствие;
			Для Каждого СтрокаЗамены Из СтрокиДляЭтойРецептуры Цикл
				КартаЗамен.Вставить(СтрокаЗамены.ВскрытаяНоменклатура, СтрокаЗамены.НоваяНоменклатура);
			КонецЦикла;
			
			// Проходим по ингредиентам и делаем замены
			Для Каждого СтрокаИнгредиента Из РецептураОбъект.Ингредиенты Цикл
				
				НоваяНоменклатура = КартаЗамен[СтрокаИнгредиента.Номенклатура];
				Если НоваяНоменклатура <> Неопределено Тогда
					СтрокаИнгредиента.Номенклатура = НоваяНоменклатура;
					БылиИзмененияВРецептуре = Истина;
				КонецЕсли;
				
			КонецЦикла;
			
			// Записываем объект только если в нем были изменения
			Если БылиИзмененияВРецептуре Тогда
				РецептураОбъект.Записать();
			КонецЕсли;
			
		КонецЦикла;
		
		Сообщить("Изменения сохранены успешно! Обновлено/создано соответствий: " + ТаблицаИзменений.Количество());
		
		// Обновляем дерево, перезапросив все данные
		ПредзаполнитьСвязиРецептурНаСервере();
