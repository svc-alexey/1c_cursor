...
Процедура СоздатьНоменклатуруНаСервере()
	...
	Для НомерПакета = 0 По КоличествоПакетов - 1 Цикл
		...
		Если ТЗ_ЗаписиВРегистр.Количество() > 0 Тогда
			НаборЗаписейПромежуточный = РегистрыСведений.MRS_СоответствиеСтаройИНовойНоменклатуры.СоздатьНаборЗаписей();
			НаборЗаписейПромежуточный.Загрузить(ТЗ_ЗаписиВРегистр);
			НаборЗаписейПромежуточный.Записать(Ложь);
		КонецЕсли;
