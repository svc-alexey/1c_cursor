...
&НаКлиенте
Процедура РецептурыПриВыводеСтроки(Элемент, ОформлениеСтроки, ДанныеСтроки)
	
	// Условное оформление строк в зависимости от статуса рецептуры
	Если ДанныеСтроки.СтатусРецептуры = "Полностью заменена" Тогда
		ОформлениеСтроки.ЦветФона = WebColors.LightGreen;
	ИначеЕсли ДанныеСтроки.СтатусРецептуры = "Частично заменена" Тогда
		ОформлениеСтроки.ЦветФона = WebColors.LightYellow;
	ИначеЕсли ДанныеСтроки.СтатусРецептуры = "Не заменена" 
		ИЛИ ДанныеСтроки.СтатусРецептуры = "Не создана" Тогда
		ОформлениеСтроки.ЦветФона = WebColors.LightCoral;
	КонецЕсли;
