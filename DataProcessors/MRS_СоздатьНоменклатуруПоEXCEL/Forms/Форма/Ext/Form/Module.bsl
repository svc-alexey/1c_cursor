...
Процедура ОбновитьДеревоРецептурНаСервере()
	...
	// Создаем новое дерево значений, чтобы гарантировать его структуру
	НовоеДерево = Новый ДеревоЗначений;
	НовоеДерево.Колонки.Добавить("ТипУзла", Новый ОписаниеТипов("Строка"));
	НовоеДерево.Колонки.Добавить("РецептураСтарая", Новый ОписаниеТипов("ДокументСсылка.питРецептура"));
	НовоеДерево.Колонки.Добавить("РецептураНовая", Новый ОписаниеТипов("ДокументСсылка.питРецептура"));
	НовоеДерево.Колонки.Добавить("НоменклатураСтарая", Новый ОписаниеТипов("СправочникСсылка.Номенклатура"));
	НовоеДерево.Колонки.Добавить("КоличествоСтарое", Новый ОписаниеТипов("Число"));
	НовоеДерево.Колонки.Добавить("НоменклатураНовая", Новый ОписаниеТипов("СправочникСсылка.Номенклатура"));
	НовоеДерево.Колонки.Добавить("КоличествоНовое", Новый ОписаниеТипов("Число"));
	НовоеДерево.Колонки.Добавить("СтатусСоответствия", Новый ОписаниеТипов("Строка"));
	
	Если Рецептуры.Количество() = 0 Тогда
		// Если нет данных, просто обновляем форму пустой, но правильной структурой дерева
		ЗначениеВДанныеФормы(НовоеДерево, "ДеревоРецептур");
		Возврат;
	КонецЕсли;
	...
	// 3. Строим дерево, используя локальную переменную НовоеДерево
	Для Каждого СтрокаПары Из Рецептуры Цикл
		
		УзелПары = НовоеДерево.Строки.Добавить();
	...
	// 4. Помещаем готовое дерево в реквизит формы
	ЗначениеВДанныеФормы(НовоеДерево, "ДеревоРецептур");
