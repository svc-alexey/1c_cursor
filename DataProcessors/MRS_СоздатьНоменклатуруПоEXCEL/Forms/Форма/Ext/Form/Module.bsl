...
Процедура СоздатьНоменклатуруНаСервере()
	...
	// Оптимизация: одним запросом найдем всю существующую номенклатуру по РАБОЧЕМУ наименованию
	...
	ЗапросПоиска.Текст = 
		...
		|	Справочник.Номенклатура КАК Номенклатура
		|ГДЕ
		|	Номенклатура.РабочееНаименование В(&МассивНаименований)";
	...
	Попытка
		...
		Для Каждого СтрокаГруппы Из ТЗДляГруппировки Цикл
			...
			Если СуществующаяНоменклатура = Неопределено Тогда
				...
				НовСтр = НоваяНоменклатураОбъектДляОбновления.ДополнительныеРеквизиты.Добавить();
