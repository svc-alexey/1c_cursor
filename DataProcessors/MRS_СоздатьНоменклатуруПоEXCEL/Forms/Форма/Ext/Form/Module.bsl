...
Процедура СоздатьНоменклатуруНаСервере()
	...
	ТЗДляГруппировки.Колонки.Добавить("ОбъемДАЛ", Новый ОписаниеТипов("Число", Новый КвалификаторыЧисла(10, 3)));
	...
	// Оптимизация: кешируем доп. свойства
	Свойство_MRS_СтараяНоменклатура = ПланыВидовХарактеристик.ДополнительныеРеквизитыИСведения.НайтиПоНаименованию("MRS_СтараяНоменклатура");
	Свойство_MRS_КодыВскрытойНоменклатуры = ПланыВидовХарактеристик.ДополнительныеРеквизитыИСведения.НайтиПоНаименованию("MRS_КодыВскрытойНоменклатуры");
	Свойство_MRS_НетОстатка = ПланыВидовХарактеристик.ДополнительныеРеквизитыИСведения.НайтиПоНаименованию("MRS_НетОстатка");
	Свойство_MRS_НоменклатураНовая = ПланыВидовХарактеристик.ДополнительныеРеквизитыИСведения.НайтиПоНаименованию("MRS_НоменклатураНовая");
	
	// Оптимизация: одним запросом найдем всю существующую номенклатуру
	...
	ЗапросСтарыхСсылок = Новый Запрос("ВЫБРАТЬ Ссылка, Код ИЗ Справочник.Номенклатура ГДЕ Код В (&Коды)");
	...
	Попытка
		...
		Для Каждого СтрокаГруппы Из ТЗДляГруппировки Цикл
			...
			Если СуществующаяНоменклатура = Неопределено Тогда
				...
				НоваяНоменклатураОбъект.УстановитьСсылкуНового(Справочники.Номенклатура.ПолучитьСсылку());
				...
				НовСтр = НоваяНоменклатураОбъект.ДополнительныеРеквизиты.Добавить();
				НовСтр.Свойство = Свойство_MRS_СтараяНоменклатура;
				НовСтр.Значение = СтрокаГруппы.КодыСтаройНоменклатуры;
				
				// MRS_КодыВскрытойНоменклатуры
				НовСтр = НоваяНоменклатураОбъект.ДополнительныеРеквизиты.Добавить();
				НовСтр.Свойство = Свойство_MRS_КодыВскрытойНоменклатуры;
				НовСтр.Значение = СтрокаГруппы.КодыВскрытойНоменклатуры;
				
				// MRS_НетОстатка
				НовСтр = НоваяНоменклатураОбъект.ДополнительныеРеквизиты.Добавить();
				НовСтр.Свойство = Свойство_MRS_НетОстатка;
				...
				Для Каждого Код Из МассивСтарыхКодов Цикл
					...
					ОбъектСтаройНоменклатуры = СсылкаНаСтарую.ПолучитьОбъект();
					ОбъектСтаройНоменклатуры.ОбменДанными.Загрузка = Истина;
					
					СтрокаСвойства = ОбъектСтаройНоменклатуры.ДополнительныеРеквизиты.Найти(Свойство_MRS_НоменклатураНовая, "Свойство");
					Если СтрокаСвойства = Неопределено Тогда
						СтрокаСвойства = ОбъектСтаройНоменклатуры.ДополнительныеРеквизиты.Добавить();
						СтрокаСвойства.Свойство = Свойство_MRS_НоменклатураНовая;
					КонецЕсли;
					СтрокаСвойства.Значение = Строка(НоваяНоменклатураОбъект.Код);
					ОбъектСтаройНоменклатуры.Записать();
				КонецЦикла;
			...
			Иначе
				...
				// MRS_СтараяНоменклатура
				ЗначениеСтарая = УправлениеСвойствами.ЗначениеСвойства(СсылкаНаНовуюНоменклатуру, Свойство_MRS_СтараяНоменклатура);
				...
				// MRS_КодыВскрытойНоменклатуры
				ЗначениеВскрытая = УправлениеСвойствами.ЗначениеСвойства(СсылкаНаНовуюНоменклатуру, Свойство_MRS_КодыВскрытойНоменклатуры);
				...
				// MRS_НетОстатка
				ТекущееЗначениеНетОстатка = УправлениеСвойствами.ЗначениеСвойства(СсылкаНаНовуюНоменклатуру, Свойство_MRS_НетОстатка);
				...
				// Обновляем свойства непосредственно в объекте
				СтрокаСвойства = ОбъектНовойНоменклатуры.ДополнительныеРеквизиты.Найти(Свойство_MRS_СтараяНоменклатура, "Свойство");
				Если СтрокаСвойства = Неопределено Тогда
					СтрокаСвойства = ОбъектНовойНоменклатуры.ДополнительныеРеквизиты.Добавить();
					СтрокаСвойства.Свойство = Свойство_MRS_СтараяНоменклатура;
				КонецЕсли;
				СтрокаСвойства.Значение = ЗначениеСтарая;
				
				СтрокаСвойства = ОбъектНовойНоменклатуры.ДополнительныеРеквизиты.Найти(Свойство_MRS_КодыВскрытойНоменклатуры, "Свойство");
				Если СтрокаСвойства = Неопределено Тогда
					СтрокаСвойства = ОбъектНовойНоменклатуры.ДополнительныеРеквизиты.Добавить();
					СтрокаСвойства.Свойство = Свойство_MRS_КодыВскрытойНоменклатуры;
				КонецЕсли;
				СтрокаСвойства.Значение = ЗначениеВскрытая;
				
				СтрокаСвойства = ОбъектНовойНоменклатуры.ДополнительныеРКвизиты.Найти(Свойство_MRS_НетОстатка, "Свойство");
				Если СтрокаСвойства = Неопределено Тогда
					СтрокаСвойства = ОбъектНовойНоменклатуры.ДополнительныеРеквизиты.Добавить();
					СтрокаСвойства.Свойство = Свойство_MRS_НетОстатка;
				КонецЕсли;
				СтрокаСвойства.Значение = НовоеЗначениеНетОстатка;
				...
				Для Каждого Код Из МассивСтарыхКодов Цикл
					...
					Если СтрокаСвойства = Неопределено Тогда
						...
						СтрокаСвойства.Свойство = Свойство_MRS_НоменклатураНовая;
					КонецЕсли;
					СтрокаСвойства.Значение = Строка(СуществующаяНоменклатура.Код);
					ОбъектСтаройНоменклатуры.Записать();
				КонецЦикла;
...
Функция СкорректироватьНаименованиеПоОбъему(Наименование, ОбъемДАЛ)
	...
	НаименованиеДляПроверки = НРег(Наименование);
	
	// Проверяем наличие объема в наименовании
	Если СтрНайти(НаименованиеДляПроверки, "0.375") > 0 
		Или СтрНайти(НаименованиеДляПроверки, "0,375") > 0
		Или СтрНайти(НаименованиеДляПроверки, "0.5") > 0 
		Или СтрНайти(НаименованиеДляПроверки, "0,5") > 0
		Или СтрНайти(НаименованиеДляПроверки, "0.7") > 0 
		Или СтрНайти(НаименованиеДляПроверки, "0,7") > 0
		Или СтрНайти(НаименованиеДляПроверки, " 1л") > 0
		Или СтрНайти(НаименованиеДляПроверки, " 1l") > 0
		Или Прав(НаименованиеДляПроверки, 2) = "1л"
		Или Прав(НаименованиеДляПроверки, 2) = "1l" Тогда
		
		Возврат Наименование;
		
	КонецЕсли;
	
	// Если объем не найден, добавляем его
	Объем = ОбъемДАЛ * 10;
	Если Объем = 0 Тогда // Не добавляем объем, если он нулевой
		Возврат Наименование;
	КонецЕсли;

	ОбъемСтрока = Формат(Объем, "ЧГ=0; ЧДЦ=3");
	
	// Удаление незначащих нулей справа
	Пока Прав(ОбъемСтрока, 1) = "0" И СтрНайти(ОбъемСтрока, ",") > 0 Цикл
		ОбъемСтрока = Лев(ОбъемСтрока, СтрДлина(ОбъемСтрока) - 1);
	КонецЦикла;
	Если Прав(ОбъемСтрока, 1) = "," Тогда
		ОбъемСтрока = Лев(ОбъемСтрока, СтрДлина(ОбъемСтрока) - 1);
	КонецЕсли;
	
	Добавка = " " + ОбъемСтрока + "л";
	МаксДлина = 150;
	
	Если СтрДлина(Наименование) + СтрДлина(Добавка) > МаксДлина Тогда
		Наименование = Лев(Наименование, МаксДлина - СтрДлина(Добавка));
	КонецЕсли;
	
	Возврат Наименование + Добавка;
