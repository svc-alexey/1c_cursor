...
Процедура СохранитьЗаменыНаСервере()
	...
	Для Каждого СтрокаДерева Из ДеревоПозицийБезЗамен.ПолучитьЭлементы() Цикл
		Если ЗначениеЗаполнено(СтрокаДерева.Замена) Тогда
			КартаЗамен.Вставить(СтрокаДерева.Номенклатура, СтрокаДерева.Замена);
		КонецЕсли;
	КонецЦикла;
	...
	Для Каждого СтрокаДерева Из ДеревоПозицийБезЗамен.ПолучитьЭлементы() Цикл
		...
		Если ЗначениеЗаполнено(СтрокаДерева.Замена) Тогда
			...
			Попытка
				...
				Если МенеджерЗаписи.Выбран() Тогда
					МенеджерЗаписи.НоваяНоменклатура = СтрокаДерева.Замена;
					МенеджерЗаписи.Записать();
