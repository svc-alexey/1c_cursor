...
Процедура СохранитьЗаменыНаСервере()
	...
	Для Каждого СтрокаТаблицы Из ТаблицаПозицийБезЗамен Цикл
		Если ЗначениеЗаполнено(СтрокаТаблицы.Замена) Тогда
			КартаЗамен.Вставить(СтрокаТаблицы.Номенклатура, СтрокаТаблицы.Замена);
		КонецЕсли;
	КонецЦикла;
	...
	Если КартаЗамен.Количество() > 0 Тогда
		Для Каждого СтрокаТаблицы Из ТаблицаПозицийБезЗамен Цикл
			Если НЕ ЗначениеЗаполнено(СтрокаТаблицы.Замена) Тогда
				ЗаменаДляТекущей = КартаЗамен[СтрокаТаблицы.Номенклатура];
				Если ЗаменаДляТекущей <> Неопределено Тогда
					СтрокаТаблицы.Замена = ЗаменаДляТекущей;
				КонецЕсли;
			КонецЕсли;
		КонецЦикла;
	КонецЕсли;
	...
	Для Каждого СтрокаТаблицы Из ТаблицаПозицийБезЗамен Цикл
		...
		Если ЗначениеЗаполнено(СтрокаТаблицы.Замена) Тогда
			...
			Попытка
				МенеджерЗаписи = РегистрыСведений.MRS_СоответствиеСтаройНовойНоменклатуры.СоздатьМенеджерЗаписи();
				МенеджерЗаписи.ВскрытаяНоменклатура = СтрокаТаблицы.Номенклатура;
				МенеджерЗаписи.Прочитать();
				
				Если МенеджерЗаписи.Выбран() Тогда
					МенеджерЗаписи.НоваяНоменклатура = СтрокаТаблицы.Замена;
					МенеджерЗаписи.Записать();
				...
				КонецЕсли;
			Исключение
				Сообщить("Не удалось записать соответствие для номенклатуры " + СтрокаТаблицы.Номенклатура + ". Ошибка: " + ОписаниеОшибки());
			КонецПопытки;
