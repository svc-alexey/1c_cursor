...
Процедура СоздатьНоменклатуруНаСервере()
	...
	ЗапросСтарыхСсылок.УстановитьПараметр("Коды", УникальныеСтарыеКоды.Ключи());
		...
		Для Каждого СтрокаГруппы Из ТЗДляГруппировки Цикл
			...
			Если СуществующаяНоменклатура = Неопределено Тогда
				...
				Для Каждого Код Из МассивСтарыхКодов Цикл
					...
					НовСтр_Старая.Свойство = Свойство_MRS_НоменклатураНовая;
					НовСтр_Старая.Значение = Строка(НоваяНоменклатураОбъект.Код);
					УправлениеСвойствами.ЗаписатьСвойстваУОбъекта(СсылкаНаСтарую, ТЗ_Старая);
				КонецЦикла;
			...
			Иначе
				...
				Для Каждого Код Из МассивСтарыхКодов Цикл
					...
					НовСтр_Старая=ТЗ_Старая.Добавить();
					НовСтр_Старая.Свойство= Свойство_MRS_НоменклатураНовая;
					НовСтр_Старая.Значение = Строка(СуществующаяНоменклатура.Код);
					УправлениеСвойствами.ЗаписатьСвойстваУОбъекта(СсылкаНаСтарую, ТЗ_Старая);
				КонецЦикла;
