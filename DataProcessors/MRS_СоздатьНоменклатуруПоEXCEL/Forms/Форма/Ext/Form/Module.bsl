...
Процедура ПоказатьПозицииБезЗаменНаСервере()
	...
	Для Каждого СтрокаДерева Из ДеревоПозицийБезЗамен.ПолучитьЭлементы() Цикл
		...
		ЗапросАвтозамены.Текст = 
			...
			|	Справочник.Номенклатура КАК Номенклатура
			|ГДЕ
			|	Номенклатура.ПЛ_НоменклатураВоВскрытойТаре = &ПЛ_НоменклатураВоВскрытойТаре";
			
		ЗапросАвтозамены.УстановитьПараметр("ПЛ_НоменклатураВоВскрытойТаре", СтрокаДерева.Номенклатура);
		
		РезультатАвтозамены = ЗапросАвтозамены.Выполнить();
		Если НЕ РезультатАвтозамены.Пустой() Тогда
			ВыборкаАвтозамены = РезультатАвтозамены.Выбрать();
			Если ВыборкаАвтозамены.Следующий() Тогда
				СтрокаДерева.АвтоЗамена = ВыборкаАвтозамены.Ссылка;
			КонецЕсли;
		КонецЕсли;
