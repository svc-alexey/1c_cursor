...
Процедура ОбновитьДеревоРецептурНаСервере()
	...
	// Защита от "потери" колонок платформой.
	// Принудительно проверяем и при необходимости добавляем колонки в дерево на форме.
	КартаКолонок = Новый Соответствие;
	КартаКолонок.Вставить("ТипУзла", Новый ОписаниеТипов("Строка"));
	КартаКолонок.Вставить("РецептураСтарая", Новый ОписаниеТипов("ДокументСсылка.питРецептура"));
	КартаКолонок.Вставить("РецептураНовая", Новый ОписаниеТипов("ДокументСсылка.питРецептура"));
	КартаКолонок.Вставить("НоменклатураСтарая", Новый ОписаниеТипов("СправочникСсылка.Номенклатура"));
	КартаКолонок.Вставить("КоличествоСтарое", Новый ОписаниеТипов("Число"));
	КартаКолонок.Вставить("НоменклатураНовая", Новый ОписаниеТипов("СправочникСсылка.Номенклатура"));
	КартаКолонок.Вставить("КоличествоНовое", Новый ОписаниеТипов("Число"));
	КартаКолонок.Вставить("СтатусСоответствия", Новый ОписаниеТипов("Строка"));
	
	Для Каждого ЭлементКарты Из КартаКолонок Цикл
		Если ДеревоРецептур.Колонки.Найти(ЭлементКарты.Ключ) = Неопределено Тогда
			// Добавляем колонку с указанием имени и заголовка
			ДеревоРецептур.Колонки.Добавить(ЭлементКарты.Ключ, ЭлементКарты.Значение, ЭлементКарты.Ключ);
		КонецЕсли;
	КонецЦикла;
	...
	Для Каждого УзелПары Из ДеревоРецептур.ПолучитьЭлементы() Цикл
		...
		Для Каждого УзелИнгредиента Из УзелПары.ПолучитьЭлементы() Цикл
			...
			Если ЗначениеЗаполнено(УзелИнгредиента.НоменклатураНовая) 
				И УзелИнгредиента.ТипУзла = "Ингредиент" 
				И ЗначениеЗаполнено(УзелИнгредиента.НоменклатураСтарая)
				И ЗначениеЗаполнено(УзелПары.РецептураНовая) Тогда
				...
				СтрокаИзменения.РецептураСсылка = УзелПары.РецептураНовая; // Используем ссылку на НОВУЮ рецептуру
