...
Процедура ПоказатьПозицииБезЗаменНаСервере()
	...
	Запрос.Текст = 
		...
		|	Ингредиенты.Ссылка КАК Рецептура,
		|	Ингредиенты.Ссылка.Наименование КАК НаименованиеРецептуры,
...
Процедура ПолучитьТаблицуПозицийБезЗамен(Команда)
	ТаблицаПозиций = ПолучитьТаблицуПозицийБезЗаменНаСервере();
	...
	// Очищаем и заполняем таблицу данных для отображения
	Данные.Очистить();
	
	Для Каждого Строка Из ТаблицаПозиций Цикл
		НоваяСтрока = Данные.Добавить();
		НоваяСтрока.СерийнаяКод = Строка.КодНоменклатуры;
		НоваяСтрока.СерийнаяНоменклатура = Строка.Номенклатура;
		НоваяСтрока.НоменклатураВоВскрытойТареКод = Строка.КодНоменклатуры;
		НоваяСтрока.ВскрытаяПродукция = Строка.Номенклатура;
		// Добавляем информацию о рецептуре в комментарий
		НоваяСтрока.Комментарий = "НЕТ ЗАМЕНЫ. Рецептура: " + Строка.НаименованиеРецептуры + " (№ " + Строка.НомерРецептуры + ")";
	КонецЦикла;
	
	Сообщить("Загружено в таблицу " + ТаблицаПозиций.Количество() + " позиций без замен для анализа.");
	
КонецПроцедуры

&НаСервере
Функция ПолучитьТаблицуПозицийБезЗаменНаСервере()
