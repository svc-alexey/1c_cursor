...
Функция ОпределитьСтатусРецептуры(СтрокаРецептуры)
	...
	Для Каждого СтараяПозиция Из СтараяНоменклатура Цикл
		...
		Запрос.Текст = 
			...
			|	MRS_СоответствиеСтаройНовойНоменклатуры.НоваяНоменклатура КАК НоваяНоменклатура
			|ИЗ
			|	РегистрыСведений.MRS_СоответствиеСтаройНовойНоменклатуры КАК MRS_СоответствиеСтаройНовойНоменклатуры
			|ГДЕ
			|	MRS_СоответствиеСтаройНовойНоменклатуры.ВскрытаяНоменклатура = &ВскрытаяНоменклатура";
...
Процедура НастроитьУсловноеОформление()
	...
	Элементы.Рецептуры.УсловноеОформление.Очистить();
	
	// Условное оформление для полностью замененных (зеленые)
	ОформлениеПолностьюЗамененных = Элементы.Рецептуры.УсловноеОформление.Элементы.Добавить();
	ОформлениеПолностьюЗамененных.Оформление.ЦветФона = WebColors.LightGreen;
	
	ПолеОтбораПолностьюЗамененные = ОформлениеПолностьюЗамененных.Отбор.Элементы.Добавить();
	ПолеОтбораПолностьюЗамененные.ЛевоеЗначение = Новый ПолеКомпоновкиДанных("Рецептуры.СтатусРецептуры");
	ПолеОтбораПолностьюЗамененные.ВидСравнения = ВидСравненияКомпоновкиДанных.Равно;
	ПолеОтбораПолностьюЗамененные.ПравоеЗначение = "Полностью заменена";
	ПолеОтбораПолностьюЗамененные.Использование = Истина;
	
	// Условное оформление для не замененных (красные)
	ОформлениеНеЗамененных = Элементы.Рецептуры.УсловноеОформление.Элементы.Добавить();
	ОформлениеНеЗамененных.Оформление.ЦветФона = WebColors.LightCoral;
	
	ПолеОтбораНеЗамененные = ОформлениеНеЗамененных.Отбор.Элементы.Добавить();
	ПолеОтбораНеЗамененные.ЛевоеЗначение = Новый ПолеКомпоновкиДанных("Рецептуры.СтатусРецептуры");
	ПолеОтбораНеЗамененные.ВидСравнения = ВидСравненияКомпоновкиДанных.Равно;
	ПолеОтбораНеЗамененные.ПравоеЗначение = "Не заменена";
	ПолеОтбораНеЗамененные.Использование = Истина;
	
	ПолеОтбораНеСозданные = ОформлениеНеЗамененных.Отбор.Элементы.Добавить();
	ПолеОтбораНеСозданные.ЛевоеЗначение = Новый ПолеКомпоновкиДанных("Рецептуры.СтатусРецептуры");
	ПолеОтбораНеСозданные.ВидСравнения = ВидСравненияКомпоновкиДанных.Равно;
	ПолеОтбораНеСозданные.ПравоеЗначение = "Не создана";
	ПолеОтбораНеСозданные.Использование = Истина;
	
	// Условное оформление для частично замененных (желтые)
	ОформлениеЧастичноЗамененных = Элементы.Рецептуры.УсловноеОформление.Элементы.Добавить();
	ОформлениеЧастичноЗамененных.Оформление.ЦветФона = WebColors.LightYellow;
	
	ПолеОтбораЧастичноЗамененные = ОформлениеЧастичноЗамененных.Отбор.Элементы.Добавить();
	ПолеОтбораЧастичноЗамененные.ЛевоеЗначение = Новый ПолеКомпоновкиДанных("Рецептуры.СтатусРецептуры");
	ПолеОтбораЧастичноЗамененные.ВидСравнения = ВидСравненияКомпоновкиДанных.Равно;
	ПолеОтбораЧастичноЗамененные.ПравоеЗначение = "Частично заменена";
	ПолеОтбораЧастичноЗамененные.Использование = Истина;
